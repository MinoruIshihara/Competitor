...
int backRun(){
  ...
  if(millis() - startTime > (2000 - cycleNum * 500)){
    startTime = millis();
    return SEEK;
  }
  return BACK;
...
int seekCup(float distance, float prevRadian, float radian, int count){
  if(distance < 40 ){
  ...
  if(millis() - startTime > 2000 && prevRadian < faceTheta && faceTheta < radian){
    motors.setSpeeds(0, 0);
    delay(200);
    rotateNum++;
    rotateTheta = getRotateTheta(rotateNum);
    nextTheta = getNextTheta(rotateNum);
    startTime = millis();
    return FACE_NEXT;
  }
...
int goNext(double radian, int deltaT){
  if (millis() - startTime > 1500) {
    startTime = millis();
    return SEEK;
  }
...
int faceNext(float prevRadian, double radian){
  if()
  return FACE_NEXT;
}
